...
export default {
  data () {
    ...
    var checkEmail = (rule, value, callback) => {
      ...
      // 正则的写法 regEmail.test(value),说明是在检查这个value是否满足条件
    ...
    var checkMobile = (rule, value, callback) => {
      ...
      // 正则的写法 regMobile.test(value),说明是在检查这个value是否满足条件
    ...
    return {
      // 获取用户列表的参数对象，对应getUserList
      // 注：这个是我们自己定义的，初始值都是随便写的
      queryInfo: {
        query: '',
        // 当前的页数
        pagenum: 1,
      ...
      addFormRules: {
        ...
        username: [
          ...
          { min: 3, max: 10, message: '用户名需要3-10个字符之间', trigger: 'blur' }
        ],
        // 验证密码是否合法
        password: [
  ...
  methods: {
    ...
    // 监听添加用户对话框的关闭，关闭的时候就触发这个回调函数
    addDialogClosed () {
      this.$refs.addFormRef.resetFields()
    },
    // 监听修改用户对话框的关闭，关闭的时候就触发这个回调函数
    editDialogClosed () {
    ...
    addUser () {
      ...
      this.$refs.addFormRef.validate(async valid => {
        ...
        // 隐藏添加用户的对话框
        this.addDialogVisible = false // 点击确定按钮后需要关闭弹出对话框
        // 重新获取用户的列表
        this.getUserList()
      })
    },
    ...
    async removeUserById (id) {
      ...
      console.log(confirmResult) // 点确定返回confirm 点取消返回cancel
      // 做个判断
